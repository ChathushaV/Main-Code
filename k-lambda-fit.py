...
from dispersion_10_0 import fit_k_invm, fit_w_invcm, branch_count, branch_names, k_space_res
...
def main():
    ...
    for i in range(len(T)):
        ...
        for i in range(len(T)):
            ...
            for j in range(branch_count):
                q_values = np.linspace(1e-12, np.pi/T_chirality, k_space_res)
                norm_q_values = normalize_q(q_values)
                cp_int_values = heat_capacity(norm_q_values, fit_w_invcm[j], T[i])
    ...
    ax4.plot(T,cp)
    ax4.set_title('Temperature Dependence of Specific Heat Capacity of CNTs')
    ax4.set_xlabel('Temperature [K]')
    ax4.set_ylabel('Specific Heat Capacity [W/m3K]')
